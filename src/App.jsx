...
export default function App() {
  ...
  const [model, setModel] = usePersistedState({
    month: monthKey,
    groups: DEFAULT_GROUPS,           // [{id,name,type,pool}]
    categories: DEFAULT_CATEGORIES,   // [{id,name,amount,groupId}]
    entries: {},                      // { [catId]: [{date,amount,memo,type:'expense'|'income'}] }
    // 과거버전 호환 필드들(salary/group 문자열)은 마이그레이션에서 처리
  });

  // --- 과거 저장본 마이그레이션 (group -> groupId, salary -> groups.salary.pool)
  useEffect(() => {
    setModel((m) => {
      let changed = false;
      let groups = m.groups;
      ...
      // 3) salary 필드를 groups.salary.pool로 반영
