...
const shiftYM = (ym, delta) => { const [y,m]=ym.split('-').map(n=>parseInt(n,10)); const d=new Date(y,m-1+delta,1); const yy=d.getFullYear(); const mm=String(d.getMonth()+1).padStart(2,'0'); return `${yy}-${mm}`; };
...
// ==== 월별 모델 훅 (로컬 스토리지 저장/로드) ====
function initialMonthlyModel(ym) {
  return { month: ym, groups: DEFAULT_GROUPS, categories: DEFAULT_CATEGORIES, entries: {} };
}
...
export default function App() {
  ...
  const signInGoogle = async () => { if (!fb.auth) return alert("먼저 연동 설정을 해주세요."); const provider = new GoogleAuthProvider(); await signInWithPopup(fb.auth, provider); };
  const signOutAll = async () => { if (fb.auth) await signOut(fb.auth); };
  const connectHouse = () => { if (!houseInput) return; const id = houseInput.trim(); setHouseId(id); localStorage.setItem("budget-houseId", id); };
  ...
  const saveSnapshot = async () => {
    ...
    try {
      ...
      setSnapshots(qs.docs.map((d) => ({ id: d.id, ...d.data() })));
      alert("스냅샷 저장 완료!");
    } catch { alert("스냅샷 저장 중 오류가 발생했습니다."); }
    finally { setIsSavingSnap(false); }
  };
  ...
  useEffect(ensureValidTab, [groups, mainTab]);
  ...
  const renameGroup = (id) => { const g=groups.find(x=>x.id===id); const name=prompt('그룹 이름 변경', g?.name||''); if (!name) return; updateGroup(id,{name}); };
  const toggleGroupType = (id) => { const g=groups.find(x=>x.id===id); if (!g) return; updateGroup(id,{ type: g.type==='salary'?'generic':'salary' }); };
  ...
  const addCategoryRow = (groupId) => { const name=prompt('새 통장 이름','새 통장'); if(!name) return; const id=`cat_${Date.now().toString(36)}`; setModel((m)=>({...m, categories:[...m.categories,{id,name,amount:0,groupId}], })); };
  const updateCategory = (id, field, value) => setModel((m)=>({ ...m, categories: m.categories.map(c=> c.id===id?{...c,[field]: field==='amount'? Number(value)||0 : value}:c ) }));
  ...
  const resetAll = () => { if(!confirm(`${ym} 데이터를 모두 초기화할까요?`)) return; setModel(initialMonthlyModel(ym)); setSelectedDate(`${ym}-01`); setMainTab('calendar'); };
  ...
  const calendarData = useMemo(()=>{
    ...
    Object.entries(model.entries||{}).forEach(([catId,arr])=>{
      (arr||[]).forEach(e=>{ if(!e?.date || !e.date.startsWith(prefix)) return; const t=(e.type||'expense')==='income'?'income':'expense'; if(!map[e.date]) map[e.date]={income:0,expense:0}; map[e.date][t]+=Number(e.amount)||0; });
    });
    return map;
  }, [model.entries, ym]);
  const monthTotals = useMemo(()=>{ let income=0,expense=0; Object.values(calendarData).forEach(v=>{income+=v.income||0; expense+=v.expense||0;}); return {income,expense,net:income-expense}; },[calendarData]);
  ...
  const recentEntries = useMemo(()=>{
    const list=[]; Object.entries(model.entries||{}).forEach(([catId,arr])=>{ const name=categories.find(c=>c.id===catId)?.name||catId; (arr||[]).forEach((e,idx)=> list.push({...e,catId,idx,catName:name})); });
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 py-4 space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select value={restoreId} onChange={(e)=>setRestoreId(e.target.value)} className="px-3 py-1.5 rounded-xl border">
                ...
                {snapshots.map((s)=>{ const t=new Date(s.createdAt); const label=`${t.getMonth()+1}/${t.getDate()} ${String(t.getHours()).padStart(2,'0')}:${String(t.getMinutes()).padStart(2,'0')}`; return <option key={s.id} value={s.id}>{label}</option>; })}
              </select>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {groups.map((g)=> (
              <button key={g.id} onClick={()=>setMainTab(g.id)} className={`px-4 py-2 rounded-xl text-sm ${mainTab===g.id?'bg-indigo-600 text-white':'bg-slate-100 hover:bg-slate-200'}`} title={g.type==='salary'?'월급 그룹':'일반 그룹'}>
                {g.name}
              </button>
            ))}
          ...
          <section className="grid lg:grid-cols-2 gap-6">
            <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow p-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <input type="number" inputMode="numeric" className="w-full sm:w-64 px-3 py-2 rounded-xl border focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="예: 3,000,000" value={activeGroup?.pool||0} onChange={(e)=>updateGroup(activeGroup.id,{pool:Number(e.target.value)||0})} />
                <span className="text-sm text-slate-500">{KRW(activeGroup?.pool||0)}</span>
              </div>
              <div className="mt-4"><button onClick={()=>addCategoryRow(activeGroup.id)} className="px-3 py-1.5 rounded-xl text-sm bg-slate-100 hover:bg-slate-200">카테고리 추가</button></div>
              <div className="mt-2 overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-left text-slate-500"><th className="py-2">통장 이름</th><th className="py-2">배정금액(원)</th><th className="py-2">비율(%)</th><th className="py-2">삭제</th></tr></thead>
                  <tbody>
                    {catsOfActive.map((c,idx)=>{
                      ...
                      return (
                        <tr key={c.id} className="border-t">
                          <td className="py-2 flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-full" style={{background:COLORS[idx%COLORS.length]}} />
                          ...
                          <td className="py-2"><input type="number" className="w-40 px-2 py-1 rounded-lg border" value={c.amount??0} onChange={(e)=>updateCategory(c.id,'amount',e.target.value)} /> <span className="ml-2 text-slate-500">{KRW(c.amount??0)}</span></td>
                          <td className="py-2">{percent}%</td>
                          <td className="py-2"><button onClick={()=>deleteCategoryRow(c.id)} className="text-xs px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200">삭제</button></td>
                        </tr>
                      );
                    })}
                    <tr className="border-t"><td className="py-2 flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-full" style={{background:COLORS[5]}} />남는 돈</td><td className="py-2">{KRW(remainPool)}</td><td className="py-2 text-slate-500">{Number(activeGroup?.pool||0)>0? Math.round((remainPool/Number(activeGroup.pool||0))*100):0}%</td><td></td></tr>
                  </tbody>
                </table>
              </div>
            </motion.div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
              {catsOfActive.map((c,idx)=>{
                ...
                const used=Math.max(0,expense-income); const remain=Math.max(0,(c.amount||0)-used);
                ...
                return (
                  <motion.div key={c.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:idx*0.03}} className="bg-white rounded-2xl shadow p-4 flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-full" style={{background:COLORS[idx%COLORS.length]}} /><h3 className="font-semibold">{c.name}</h3></div>
...
function MonthCalendar({ ym, data, selectedDate, onSelectDate }) {
  ...
  const first=new Date(y,m-1,1); const firstWeekday=first.getDay(); const daysInMonth=new Date(y,m,0).getDate();
  const cells=[]; for(let i=0;i<firstWeekday;i++) cells.push(null); for(let d=1; d<=daysInMonth; d++) cells.push(d); while(cells.length%7!==0) cells.push(null);
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1 text-xs">
        ...
        {cells.map((d,idx)=>{
          ...
          const dd=String(d).padStart(2,'0'); const iso=`${ym}-${dd}`; const sum=data[iso]||{income:0,expense:0}; const isToday=iso===today; const isSelected=iso===selectedDate;
          return (
            <button key={idx} onClick={()=>onSelectDate?.(iso)} className={`h-20 p-2 rounded-lg text-left bg-white border ${isSelected? 'border-indigo-500 ring-2 ring-indigo-200':'border-slate-200'} ${isToday? 'shadow-inner':''}`} title={`${iso} 수입 ${sum.income.toLocaleString('ko-KR')}원 / 지출 ${sum.expense.toLocaleString('ko-KR')}원`}>
...
function GlobalEntryForm({ categories, selectedDate, onAdd }) {
  ...
  const submit=(e)=>{ e.preventDefault(); const a=Number(amount)||0; if(!catId) return alert('통장을 선택하세요.'); if(a<=0) return alert('금액을 입력하세요.'); onAdd(catId, {date, amount:a, memo:memo.trim(), type}); setAmount(0); setMemo(''); };
  return (
    <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-6 gap-2">
      <select className="px-3 py-2 rounded-xl border" value={catId} onChange={(e)=>setCatId(e.target.value)}>{categories.map(c=> (<option key={c.id} value={c.id}>{c.name}</option>))}</select>
      <input type="date" className="px-3 py-2 rounded-xl border" value={date} onChange={(e)=>setDate(e.target.value)} />
      <select className="px-3 py-2 rounded-xl border" value={type} onChange={(e)=>setType(e.target.value)}><option value="expense">지출</option><option value="income">수입</option></select>
...
function EntryForm({ onAdd }) {
  const d=new Date(); const yyyy=d.getFullYear(); const mm=String(d.getMonth()+1).padStart(2,'0'); const dd=String(d.getDate()).padStart(2,'0'); const defaultDate=`${yyyy}-${mm}-${dd}`;
  const [date,setDate]=useState(defaultDate); const [amount,setAmount]=useState(0); const [memo,setMemo]=useState(''); const [type,setType]=useState('expense');
  const submit=(e)=>{ e.preventDefault(); const a=Number(amount)||0; if(a<=0) return alert('금액을 입력하세요'); onAdd({date,amount:a,memo:memo.trim(),type}); setAmount(0); setMemo(''); };
  return (
...
function CategoryEntriesTable({ catId, entries, onRemove }) {
  return (
    <div className="mt-3 overflow-x-auto">
      <table className="w-full text-sm">
        <thead><tr className="text-left text-slate-500"><th className="py-1">날짜</th><th className="py-1">유형</th><th className="py-1">금액</th><th className="py-1">메모</th><th className="py-1">삭제</th></tr></thead>
        <tbody>
          {entries.map((e,i)=> (
...
function DateEntriesTable({ date, categories, entries, onRemove }) {
  const rows=[]; Object.entries(entries||{}).forEach(([catId,arr])=>{ const name=categories.find(c=>c.id===catId)?.name||catId; (arr||[]).forEach((e,idx)=>{ if(e.date===date) rows.push({...e, catName:name, idx, catId}); }); });
  if(rows.length===0) return <div className="text-center text-slate-400">이 날짜의 기록이 없습니다.</div>;
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead><tr className="text-left text-slate-500"><th className="py-1">통장</th><th className="py-1">유형</th><th className="py-1">금액</th><th className="py-1">메모</th><th className="py-1">삭제</th></tr></thead>
        <tbody>
          {rows.map((e)=> (
